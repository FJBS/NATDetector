...
BOOL CSplitterBar::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	...
	if(IsCursorOverSplitter(ptCursor))
	{
		...
		if ( m_bHorizontal )
			::SetCursor( ::LoadCursor(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDC_CURSOR_HSPLIT)) );
		else
			::SetCursor( ::LoadCursor(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDC_CURSOR_VSPLIT)) );
